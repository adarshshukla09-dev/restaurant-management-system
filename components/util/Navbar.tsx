...
function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Kitchen", href: "/kitchen", icon: Flame, color: "hover:text-rose-500" },
    { name: "Cashier", href: "/cashier", icon: Wallet, color: "hover:text-blue-500" },
  ];
      ...
      <nav className="max-w-7xl mx-auto h-16 rounded-2xl border border-white/20 bg-white/80 backdrop-blur-md shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] px-6 flex items-center justify-between pointer-events-auto">
        ...
        <div className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => {
            ...
            const fullHref = tableId && !link.href.includes('kitchen') && !link.href.includes('cashier') 
        ...
        <div className="md:hidden flex items-center gap-3">
           {user && (
              <div className="h-8 w-8 rounded-lg overflow-hidden border border-slate-200">
                <img src={userImage} alt="User" className="object-cover h-full w-full" />
              </div>
           )}
          <Sheet>
            <SheetTrigger asChild>
              <Button variant="ghost" size="icon" className="rounded-xl bg-slate-50 text-slate-900">
                <Menu className="h-6 w-6" />
              </Button>
            </SheetTrigger>
            <SheetContent side="right" className="w-full sm:w-87.5 border-none rounded-l-[3rem]">
              <SheetHeader className="text-left mb-10">
                <SheetTitle className="text-3xl font-black italic">MENU</SheetTitle>
              </SheetHeader>
                  ...
                  <Link key={link.name} href={link.href} className="group">
                    <div className="flex items-center justify-between p-4 rounded-2xl hover:bg-slate-50 transition-all">
                      <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-xl bg-slate-100 group-hover:bg-white group-hover:shadow-md transition-all`}>
                          <link.icon className="text-slate-600" size={20} />
                        </div>
                        <span className="text-lg font-bold text-slate-800">{link.name}</span>
                      </div>
                      <ArrowRightIcon className="text-slate-300 group-hover:text-orange-500 group-hover:translate-x-1 transition-all" size={18} />
                    </div>
                  </Link>
                ))}
              </div>
